...
class FacebookLoginSerializer(serializers.Serializer):
    ...
    def validate_access_token(self, value):
        ...
        Validate the access token and retrieve or create the user.
        """
        user_data = self.verify_facebook_token(value)
        user = self.get_or_create_user(user_data)
        self.validated_data["user"] = user
        return value
    ...
    def verify_facebook_token(self, access_token):
        ...
        response = requests.get(url)
        if response.status_code != 200:
            logger.error("Facebook token verification failed: %s", response.text)
            raise ValidationError(_("Invalid Facebook token provided."))
